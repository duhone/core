...
//Is a StrictWeakOrdering, but uses equality instead of equivalence. Is also Regular. In addition is comparable using <= and >=.
//And holds following additional properties
//antitymytry: if x<=y and y<=x then x=y
//totality: One of x<=y or y<=x must be true
//transitivity: if x<=y and y<=z then x<=z must be true
...
//Can only be passed through once(i.e. a getting a new iterator from begin would not allow you to re-read previous values).
