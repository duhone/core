...
namespace CR
{
	namespace Core
	{
		...
		{
			friend bool operator!=(const T& a_arg1, const T& a_arg2)
			{
		...
		struct DefaultOperatorsTotallyOrdered : public DefaultOperatorsRegular<T>, public DefaultOperatorsStrictWeakOrdering<T>
		{
			friend bool operator<=(const T& a_arg1, const T& a_arg2)
			{
				return  !(a_arg2 < a_arg1);
		...
		//the (const T&, T&&) variants of this function if your type is communitive. You can also probably do better for classes
		//like string where using the larger of the 2 args as the left arg could yeild improved performance.
		//If your class is very small, then the ineffeciancy of the (const T&, T&&) variant is probably irellevant.
